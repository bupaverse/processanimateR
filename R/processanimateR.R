...
#' @param animation_renderer Whether to use Graphviz (\code{\link{renderer_graphviz}}) to layout and render the process map,
...
#' @param animation_jitter The magnitude of a random coordinate translation, known as jitter in scatterplots, which is added to each token.
...
#' @param animation_repeat_delay The seconds to wait before one repetition of the animation.
...
#' @param token_shape The (fixed) SVG shape to be used to draw tokens. Can be either 'circle' (default), 'rect' or 'image'.
...
animate_process <- function(eventlog,
                            ...
                            animation_renderer = renderer_graphviz(),
                            ...
                            animation_repeat = 1,
                            animation_repeat_delay = 0.5,
                            #TODO, turn into some kind of token_aes function
                            token_size = NULL,
                            ...
                            #TODO, turn into animation_events function
  ...
  rendered_process <- animation_renderer(processmap, width, height)
  ...
  x <- list(
    ...
    onclick_activity_decorate = htmlwidgets::JS('function(node, selected) {
          if (selected) {
            node.select("path")
              .attr("stroke-width", "3")
              .attr("stroke", "black");
    ...
    onclick_token_decorate = htmlwidgets::JS('function(node, selected) {
          if (selected) {
    ...
    processmap_renderer = attr(animation_renderer, "name")
  )
